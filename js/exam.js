...
function renderQuestionNavigation() {
  ...
  questions.forEach((_, index) => {
    ...
    rightSlot.className = 'w-6 text-right';
...
function updateQuestionNavigation() {
  ...
  buttons.forEach(btn => {
    ...
    btn.classList.remove('bg-gray-200', 'bg-success', 'bg-warning', 'custom-current', 'hover:bg-gray-300', 'hover:bg-success/90', 'hover:bg-warning/90');
    
    btn.classList.add('transition-colors', 'duration-200');
    ...
    if (flaggedQuestions.has(index)) {
      ...
      rightSlot.innerHTML = '🚩';
      rightSlot.className = 'w-6 text-right text-lg';
    }
